...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match opts.command {
        ...
        Command::DisablePeer { interface } => enable_or_disable_peer(&interface, &conf, false, opts.network)?,
        Command::EnablePeer { interface } => enable_or_disable_peer(&interface, &conf, true, opts.network)?,
